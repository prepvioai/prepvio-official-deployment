...
function Payment() {
    ...
    if (activeTab === 'paid-bills') {
      const fetchHistory = async () => {
        try {
          ...
          const res = await axios.get(`${config.API_BASE_URL}/api/payment/history`, { withCredentials: true });
          if (res.data.success) {
  ...
  const validatePromoCode = async (planId) => {
    ...
    try {
      ...
      if (data.valid) {
        ...
        try {
          const orderData = await axios.post(`${config.API_BASE_URL}/api/payment/create-order`, { planId, promoCode });
          setOrderDetails({
  ...
  const handlePlanSelect = async (planId) => {
    ...
    try {
      const { data } = await axios.post(`${config.API_BASE_URL}/api/payment/create-order`, { planId });
      setOrderDetails({
        originalAmount: data.originalAmount,
        upgradeDiscount: data.upgradeDiscount || 0,
        discountAmount: data.discountAmount || 0,
